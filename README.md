...
* **심각한 공수 낭비**: 고객의 "이번 부가세 신고 대상인가요?" 문의 1건당 [고객 식별 → 세무 ERP(TA) 검색 → 국세청 홈택스 과세유형 수동 조회 → 세금계산서 발급 권한 확인 → 메신저 응대문 타이핑]까지 **건당 평균 3분(180초)** 소요.
...
유입된 고객 정보(이름/전화번호)를 바탕으로 내부 세무회계 ERP DB를 고속 탐색(Pandas).

대표자명, 상호명, 사업자번호, 개업일자, 업종코드를 자동 정합성 검증 후 파싱.
Module 2. Gemini Multimodal Vision API 기반 사업자등록증 OCR
사업자등록증 서류 이미지를 업로드하면 최신 Gemini Vision 엔진이 메타데이터를 구조화된 JSON 객체로 즉시 추출.

비정형 이미지 데이터 유입 시에도 수동 타이핑 없는 무중단 파이프라인 구현.

Module 3. 국세청 공공데이터 API 실시간 동기화
행정안전부/국세청 공식 사업자등록정보 진단 API를 호출하여 실시간 사업자 상태(계속/폐업) 및 과세유형(일반/간이/면세) 동기화.

공공데이터 API 응답 실패 시 내부 DB로 자동 페일오버(Fallback)하는 고가용성 구조 설계.

Module 4. 부가가치세법 기반 분기 및 Gemini 상담 생성
세법 근거 프롬프트 엔지니어링: 판별된 과세유형과 세금계산서 발급 권한에 맞추어 전문적이고 친절한 1:1 세무 응대 메시지 실시간 생성.

법적 면책 조항 및 세무사 최종 검토 안내 문구를 기본 포함하여 리스크 통제.

Module 5. 전문가 전용 백오피스 & 1-Click Clipboard UX
파싱된 사업자 메타데이터를 백오피스 대시보드에 카드 형태로 시각화.

세무 프로그램(더존 Smart A, 세무사랑 등) 전표 메모란에 즉시 붙여넣을 수 있는 1-Click 복사 자바스크립트 컴포넌트 탑재.

핵심 지표 (KPIs),As-Is (수동 처리),To-Be (AI Pipeline),개선 효과
인바운드 CS 1건당 응대 시간,180초 (3분),1.5초,99.2% 리드타임 단축
과세유형 및 신고대상 판별 정확도,휴먼 에러 위험 존재,국세청 실시간 API 기반,100% 정합성 유지
...
Frontend / Framework: Streamlit, HTML5, Custom CSS & JavaScript

AI & Multimodal Engine: Google GenAI SDK (gemini-2.0-flash, gemini-1.5-flash)

External API: 국세청_사업자등록정보 진단 및 조회 서비스 (data.go.kr)

Data Preprocessing: Pandas, OpenPyXL, Pillow (PIL), Python-dotenv
